...
$remoteCmd = "sed -i 's/\r`$//' '$RemoteDir/bootstrap-server.sh' '$RemoteDir/bootstrap.env' && chmod +x '$RemoteDir/bootstrap-server.sh' && if sudo -n true 2>/dev/null; then sudo bash '$RemoteDir/bootstrap-server.sh'; else echo '$DEPLOY_PASSWORD' | sudo -S bash '$RemoteDir/bootstrap-server.sh'; fi"
