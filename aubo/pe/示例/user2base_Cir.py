...
def robot_init():
    ...
    try:
        ...
        if result != RobotErrorType.RobotError_SUCC:
            ...
            # 移动到初始位置
            joint0 = (0, 0, 0, 0, 0, 0)
            libpyauboi5.move_joint(handle_move, joint0, True)
...
def test_count(count, pos_by_tool, rpy_by_tool_deg):
    """
    测试函数，将工具坐标系位置和RPY角度转换为关节角度

    Args:
        count: 测试次数
        pos_by_tool: 工具坐标系下的位置 (x, y, z)
        rpy_by_tool_deg: 工具坐标系下的RPY角度 (roll, pitch, yaw)，单位：度

    Returns:
        dict: 包含关节角度、位置和姿态的目标点
    """
    # 将角度从度转换为弧度
    rpy_by_tool = (
        rpy_by_tool_deg[0] / 180.0 * pi,
        rpy_by_tool_deg[1] / 180.0 * pi,
        rpy_by_tool_deg[2] / 180.0 * pi
    )

    # 将RPY转换为四元数
    ori_by_tool = libpyauboi5.rpy_to_quaternion(handle_move, rpy_by_tool)

    print("工具坐标系姿态（四元数）:")
    print(f"  w: {ori_by_tool[0]:.6f}")
    print(f"  x: {ori_by_tool[1]:.6f}")
    print(f"  y: {ori_by_tool[2]:.6f}")
    print(f"  z: {ori_by_tool[3]:.6f}")

    # 用户坐标系定义
    user_coord = {
        'coord_type': RobotCoordType.Robot_Base_Coordinate,
        'calibrate_method': RobotCoordCalMethod.CoordCalMethod_xOxy,
        'calibrate_points': {
            "point1": tuple(deg * math.pi / 180 for deg in
                            [-75.093279, 28.544643, -114.313905, -62.769247, -87.343517, -27.888262]),
            "point2": tuple(deg * math.pi / 180 for deg in
                            [-89.239837, 23.936171, -122.299277, -65.208902, -85.011123, -41.87417]),
            "point3": tuple(deg * math.pi / 180 for deg in
                            [-77.059212, 35.509518, -101.108547, -56.433133, -87.006734, -29.827440])
        },
        'tool_desc': {
            "pos": (0.0, 0.0, 0.0),
            "ori": (1.0, 0.0, 0.0, 0.0)
    ...
    # 工具定义
    tool_rpy = (30.0 / 180.0 * pi, 20.0 / 180.0 * pi, 0.0 / 180.0 * pi)
    tool_ori = libpyauboi5.rpy_to_quaternion(handle_move, tool_rpy)

    print("\n工具定义:")
    print(f"  位置: (0.1, 0.05, 0.1)")
    print(f"  姿态（四元数）: w={tool_ori[0]:.6f}, x={tool_ori[1]:.6f}, y={tool_ori[2]:.6f}, z={tool_ori[3]:.6f}")

    user_tool = {
        "pos": (0.1, 0.05, 0.1),
        "ori": (tool_ori[0], tool_ori[1], tool_ori[2], tool_ori[3])
    }

    # 工具坐标系转基坐标系，工具在用户坐标系下的位姿转换为法兰在基坐标系下的位姿
    base_pos_ori = libpyauboi5.user_to_base(handle_move, pos_by_tool, ori_by_tool, user_coord, user_tool)

    print("\n基坐标系下的位置和姿态:")
    print(
        f"  位置: x={base_pos_ori['pos'][0]:.6f}, y={base_pos_ori['pos'][1]:.6f}, z={base_pos_ori['pos'][2]:.6f}")
    print(
        f"  姿态（四元数）: w={base_pos_ori['ori'][0]:.6f}, x={base_pos_ori['ori'][1]:.6f}, y={base_pos_ori['ori'][2]:.6f}, z={base_pos_ori['ori'][3]:.6f}")

    # 逆运动学求解关节角度
    target_joint = libpyauboi5.inverse_kin(
        handle_move,
    ...
    # 添加等待时间，让逆解完成
    # time.sleep(5)  # 等待500毫秒


    # print(f"\n生成的关节角度（弧度）:")
    # for i, angle in enumerate(target_joint['joint']):
    #     print(f"  关节{i + 1}: {angle:.6f} rad ({angle * 180 / pi:.2f}°)")

    print(f"\n目标位置: {target_joint['pos']}")
    print(f"目标姿态: {target_joint['ori']}")

    # 移动机械臂到目标位置
    # print("\n开始移动机械臂...")
    # libpyauboi5.move_joint(handle_move, target_joint['joint'], True)
    # print("机械臂移动完成")
...
if __name__ == '__main__':
    ...
    robot_init()
    # 方案
    # 使用用户坐标系转基坐标系的方式进行对应点位生成，生成后使用moveTrack的方式调用圆弧运动

    logger.info("test -------------main---------------- start")

    # 定义工具坐标系下的位置和RPY角度（度）
    pos_by_tool = (-0.19, -0.71, 0.33)  # x, y, z
    rpy_by_tool_deg = (-150.0, -19.0, -70.0)  # roll, pitch, yaw (度)
    # 调用函数
    result = test_count(1, pos_by_tool, rpy_by_tool_deg)
    time.sleep(1)
    rpy_by_tool_deg = (-150.0, -37.0, -80.0)
    result2 = test_count(1, pos_by_tool, rpy_by_tool_deg)
    time.sleep(1)
    rpy_by_tool_deg = (-150.0, -50.0, -90.0)
    result3 = test_count(1, pos_by_tool, rpy_by_tool_deg)


    # 必须使用moveJoint运行到move_track轨迹的第一个点
    libpyauboi5.move_joint(handle_move,result["joint"], True)
    # 使用圆弧运动运行
    # 清除全局路点
    ...
    # 添加路点2
    joint_radian = result2["joint"]
    robot.add_waypoint(joint_radian)
    # 添加路点3
    joint_radian = result3["joint"]
    robot.add_waypoint(joint_radian)
    # 设置圆运动圈数
    robot.set_circular_loop_times(0)
    # 圆弧运动
    result = robot.move_track(RobotMoveTrackType.ARC_CIR)
    if result != RobotErrorType.RobotError_SUCC:
        logger.info("圆弧运动失败。")
    else:
        logger.info("圆弧运动成功。")
